...
import { backendGet, backendDelete, backendPut } from '@/lib/backend-api';
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await backendPut(`/api/intelligence/profiles/${params.id}`, body, extractProjectHeaders(req));
    return NextResponse.json(data);
  } catch (err: any) {
    return NextResponse.json({ success: false, error: err.message }, { status: 500 });
  }
